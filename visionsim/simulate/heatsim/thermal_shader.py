...
def _append_temperature_aov_nodes(mat: Any, aov_name: str) -> None:
    """Append ``Attribute("sim_temperature") → ShaderNodeOutputAOV(aov_name)`` to *mat*.
    ...
    try:
        ...
        _log.debug("Could not add ShaderNodeAttribute to %r: %s", mat.name, exc)
        return

    attr_node.attribute_name = "sim_temperature"
    attr_node.attribute_type = "GEOMETRY"
    attr_node.location = (0.0, -400.0)

    try:
        aov_node = nodes.new("ShaderNodeOutputAOV")
    except Exception as exc:
        _log.debug("Could not add ShaderNodeOutputAOV to %r: %s", mat.name, exc)
